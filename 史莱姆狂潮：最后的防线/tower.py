...
class Tower():
    ...
    def __init__(self, x, y, rank, Diff):
        # 初始化塔的位置、类型、属性、升级、攻击参数等
        # rank 决定塔的种类（如飞镖猴、忍者猴、火焰塔等），
        # Diff 决定游戏难度（如简单、中等、困难），影响价格等
        self.x, self.y = x, y
...
# 根据塔的类型绘制不同的图片或形状
# 某些塔有旋转动画
        if self.rank not in [1,2, 3, 4, 5, 6, 7,8, 9]:
            pygame.draw.rect(gameDisplay, self.color, (self.x, self.y, self.width, self.height),0)
        elif self.rank == 1:
            gameDisplay.blit(pygame.transform.scale(Images["Tower1"], (80, 80)), (self.x-30, self.y-30))
        elif self.rank == 5:
            gameDisplay.blit(pygame.transform.scale(Images["Tower5"], (50, 61)), (self.x-10, self.y-30))
        elif self.rank == 6:
            gameDisplay.blit(pygame.transform.scale(Images["Tower6"], (38, 38)), (self.x-5, self.y-10))
        elif self.rank == 8:
            gameDisplay.blit(pygame.transform.scale(Images["Tower8"], (80, 80)), (self.x-30, self.y-30))
        elif self.rank == 7:
            ...
            gameDisplay.blit(pygame.transform.scale(Images["IceTower"], (60, 60)), (self.x-10, self.y-20))
        elif self.rank == 2:
            ...
            img = pygame.transform.rotate(pygame.transform.scale(Images["Cannon"], (50, 50)), self.angle)
            gameDisplay.blit(img, (self.x-int(img.get_rect().size[0]/2)+15, self.y-int(img.get_rect().size[1]/2)+10))
        else:
...
# 处理与怪物的碰撞和伤害
        ...
        if self.rank == 6 and self.cooldown <= 0:
            if not self.autoCollect:
                ...
                while run:
                    pos = (
    self.x + int(self.width/2) + random.randint(-int(self.range), int(self.range)),
    self.y + int(self.height/2) + random.randint(-int(self.range), int(self.range))
)
